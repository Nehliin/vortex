...
pub(crate) struct TokenStore {
    ...
    received_tokens: AHashMap<Ipv4Addr, (Instant, Bytes)>,
...
impl TokenStore {
    ...
    pub fn get_token(&self, ip: Ipv4Addr) -> Option<&Bytes> {
        // TODO: Remove the tokens as well at some point
        self.received_tokens
            .get(&ip)
            .filter(|(inserted_at, _token)| {
                Instant::now() - *inserted_at <= (SECRET_REFRESH_TIME * 2)
            })
            .map(|(_, token)| token)
    }

    #[inline]
    pub fn store_token(&mut self, ip: Ipv4Addr, token: Bytes) {
        self.received_tokens.insert(ip, (Instant::now(), token));
    }
}
