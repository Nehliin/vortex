...
other tool used by the project — no manual `rustup`/`apt`/`brew` steps
required.
...
For hands-off activation whenever you `cd` into the repo, install
[direnv](https://direnv.net/) and
[`nix-direnv`](https://github.com/nix-community/nix-direnv), then create
...
Every terminal session inside the repo now enters the dev shell
transparently.
...
All commands below are run **inside `nix develop`**.
...
cargo build -p vortex-cli    # build only the CLI
```

### Run the CLI

```bash
cargo run -p vortex-cli -- --help
```

Or via the flake (release binary, no dev shell needed):
...
```bash
cargo nextest run peer_comm            # protocol unit tests (no containers)
cargo nextest run --workspace          # all unit tests
```

### Integration tests

Integration tests spin up Transmission containers; you need Podman or Docker
on the host (Nix does not provide the container runtime).

```bash
just test              # generates assets, starts containers, runs nextest
```
