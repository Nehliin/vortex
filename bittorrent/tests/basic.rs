...
use vortex_bittorrent::{Torrent, generate_peer_id, EventLoopCommand};
...
fn basic_seeded_download() {
    ...
    // Explicitly use IP v4 address format
    let peer_addr = std::net::SocketAddrV4::new(std::net::Ipv4Addr::new(127, 0, 0, 1), 51413);
    tx.send(EventLoopCommand::ConnectToPeer(peer_addr)).unwrap();
    ...
    std::thread::spawn(move || {
        ...
        let _ = tx_clone.send(EventLoopCommand::Stop);
    });
